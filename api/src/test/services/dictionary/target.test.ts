...
import { Target } from './../../../app/dictionary/models/target';

const expect = chai.expect;
chai.use(chaiHttp);

describe('POST: /api/dictionary/target/add route to insert data', () => {
  basicSetup();
  it('should create and return a new target item', () => {
    let response: Response | any;
    // 1) First, call the API
    return (
      chai
        .request(myApp)
        .post('/api/dictionary/target/add')
        .send({ name: 'test2' })
        .then((_res) => {
          response = _res;
          ...
          return Target.findById(response.body._id);
        })
